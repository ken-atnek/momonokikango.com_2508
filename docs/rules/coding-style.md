...
- フォント指定を共通化する場合は、既存の `src/styles/_variable.scss` / `src/styles/_mixin.scss` の構成に合わせる
...
- 見出し用フォントを追加する場合は、既存の変数・mixin構成に合わせて追加する
...
- 例: `BlockServiceList.tsx` は `styles.blockServiceList`、SCSS側は `.blockServiceList {}`
