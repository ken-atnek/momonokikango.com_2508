...
{
  "scripts": {
    "build:demo": "NEXT_PUBLIC_IS_REAL_PROD=false NEXT_PUBLIC_METADATA_BASE=https://demo-momonokikango.tuna-pic.co.jp/ next build",
    "build:prod": "NEXT_PUBLIC_IS_REAL_PROD=true NEXT_PUBLIC_METADATA_BASE=https://momonokikango.com/ next build"
  }
}
```
...
- 既存の `src/styles/_variable.scss` / `src/styles/_mixin.scss` の構成に合わせてフォント用 `mixin` を作成
